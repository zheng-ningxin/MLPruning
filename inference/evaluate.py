...
def main():
    ...
    if args.block_path:
        ...
        for layer_id in range(n_layers):
            ...
            _layer.attention.output.LayerNorm.weight.data.copy_(_layer2.attention.output.LayerNorm.weight.data)
            _layer.attention.output.LayerNorm.bias.data.copy_(_layer2.attention.output.LayerNorm.bias.data)
            ...
            _layer.output.LayerNorm.weight.data.copy_(_layer2.output.LayerNorm.weight.data)
            _layer.output.LayerNorm.bias.data.copy_(_layer2.output.LayerNorm.bias.data)
        norm_model.bert.pooler.dense.weight.data.copy_(model.bert.pooler.dense.weight.data)
        norm_model.bert.pooler.dense.bias.data.copy_(model.bert.pooler.dense.bias.data)
        norm_model.classifier.weight.data.copy_(model.classifier.weight.data)
        norm_model.classifier.bias.data.copy_(model.classifier.bias.data)
        # setattr(norm_model.bert, 'embeddings', model.bert.embeddings)
        embedding_module = norm_model.bert.embeddings
        _embedding_module = model.bert.embeddings
        for name, module in embedding_module.named_modules():
            if name =='':
                continue
            print('Load embedding modules', name)
            _module = getattr(_embedding_module, name)
            module.load_state_dict(_module.state_dict())
        # norm_acc = evaluate(args, norm_model, tokenizer)
        # train(args, train_dataset, model, tokenizer)
        norm_acc = evaluate(args, norm_model, tokenizer)
        print('Accuracy of the normal model', norm_acc)
        dummy_input = get_dummy_input(args, model, tokenizer)
        # debug(norm_model, model, dummy_input)
        export_model(norm_model)
        torch.save(head_pruner_cfg, 'head_prune_cfg')
        import pdb; pdb.set_trace()
